...
install_option_staticip() {
    netpackage_config=$(
        whiptail --backtitle "VisionPro NVR First Boot Script - October 2023" --title "Network Package" --yesno "Does this location have a ProVision Network Package?" 16 60 3>&2 2>&1 1>&3
    )
    netpackage_config=$?
}
...
install_option_ipmi() {
    ...
    if [ "$netpackage_config" = "0" ]; then
        ipmi_config=$(
            whiptail --backtitle "VisionPro NVR First Boot Script - October 2023" --title "IPMI Configuration" --yesno "Do you wish to configure the IPMI? This configures the Static IP, IPMI hostname, and users." 16 60 3>&2 2>&1 1>&3
        )
    else
        ipmi_config=$(
            whiptail --backtitle "VisionPro NVR First Boot Script - October 2023"--title "IPMI Configuration" --yesno "Do you wish to configure the IPMI? This configures the IPMI hostname and users." 16 60 3>&2 2>&1 1>&3
        )
    fi
    ipmi_config=$?
}
...
set_hostname() {
    ...
    hostname=$(whiptail --backtitle "VisionPro NVR First Boot Script - October 2023" --title "Configure Hostname" --inputbox "Enter desired hostname for this NVR:" 16 100 "$brandhost" 10 30 3>&1 1>&2 2>&3)
    exitstatus=$?
    ...
    if [ "$exitstatus" = "1" ]; then
        ...
        whiptail --ok-button Next --backtitle "VisionPro NVR First Boot Script - October 2023" --title "Configure Hostname" --msgbox "Hostname set to $new_hostname" 16 60
    fi
}

brand_convert() {
    whiptail --backtitle "VisionPro NVR First Boot Script - October 2023" --title "Brand Conversion" --msgbox "Running brand conversion. Please wait..." 16 60
    ...
    whiptail --backtitle "VisionPro NVR First Boot Script - October 2023" --ok-button "Next" --title "Brand Conversion" --msgbox "Brand conversion complete." 16 60
}
...
do_ipmi_config() {
    ...
    ipmicfg -fru PN "VisionPro Network Video Recorder"
    ipmicfg -fru PV "Generation 4"
    ipmicfg -fru "ProVision Distribution"
    ipmicfg -fru CT 05h
